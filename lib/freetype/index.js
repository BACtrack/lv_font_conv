...
module.exports.glyph_render = function (face, code, opts = {}) {
  ...
  for (let y = 0; y < g_h; y++) {
    ...
    for (let x = 0; x < g_w; x++) {
      if (pixel_mode === m.FT_PIXEL_MODE_MONO) {
      ...
      } else if (pixel_mode === m.FT_PIXEL_MODE_GRAY4) {
        let value = m.getValue(row_start + ~~(x / 2), 'i8');
        let gray4;
        if (x % 2) {
          gray4 = int8_to_uint8(value) & 0x0F; // Extract the lower 4 bits
        } else {
          gray4 = (int8_to_uint8(value) >> 4) & 0x0F; // Extract the upper 4 bits
        }
        // Scale the 4-bit value to 8-bit range (0-255)
        line.push(gray4 * 17); // 17 = 255 / 15
        ...
        line.push(alpha);
      } else if (pixel_mode === m.FT_PIXEL_MODE_GRAY){
        let value = m.getValue(row_start + x, 'i8');
        line.push(int8_to_uint8(value));
      } else {
        throw new Error('unsupported input pixel mode');
      }
    }
